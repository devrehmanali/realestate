...
export default function SessionSidebar({ activeSessionId }: SessionSidebarProps) {
  ...
  const { data: conversations, isPending } = useQuery({
    ...
    refetchInterval: 10000, // Refresh every 10s to pick up new sessions
  });
    ...
    <div className="flex flex-col h-[calc(100vh-4rem)] bg-white border-r border-slate-100 font-inter">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-6 space-y-2 scrollbar-thin scrollbar-thumb-slate-100">
        <div className="flex items-center justify-between px-2 mb-3">
          <h3 className="text-[11px] font-bold text-slate-400 uppercase tracking-[0.1em]">History</h3>
          ...
          conversations?.data?.map((conv: any) => {
            ...
            return (
              ...
              >
                ...
                >
                  <MessageSquare
                    className={`w-4 h-4 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-indigo-500'}`}
                  />
                </div>
