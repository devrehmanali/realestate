      ...
      <div className="max-w-4xl mx-auto">
        ...
        {isPending ? (
          ...
          <div className="space-y-6">
            {/* Preferences Summary */}
            {history?.metadata_filters && Object.keys(history.metadata_filters).length > 0 && (
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <h3 className="font-outfit font-bold text-lg mb-4 flex items-center gap-2">
                   Your Current Preferences
                </h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                   {history.metadata_filters.city && (
                      <div className="flex items-center gap-3 p-3 bg-blue-50 rounded-xl">
                        <MapPin className="text-blue-500 w-5 h-5" />
                        <div>
                          <p className="text-[10px] uppercase text-blue-400 font-bold">City</p>
                          <p className="text-sm font-bold text-blue-900">{history.metadata_filters.city}</p>
                        </div>
                      </div>
                   )}
                   {history.metadata_filters.max_price && (
                      <div className="flex items-center gap-3 p-3 bg-green-50 rounded-xl">
                        <DollarSign className="text-green-500 w-5 h-5" />
                        <div>
                          <p className="text-[10px] uppercase text-green-400 font-bold">Budget</p>
                          <p className="text-sm font-bold text-green-900">{history.metadata_filters.max_price.toLocaleString()}</p>
                        </div>
                      </div>
                   )}
                   {history.metadata_filters.bedrooms && (
                      <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-xl">
                        <Bed className="text-orange-500 w-5 h-5" />
                        <div>
                          <p className="text-[10px] uppercase text-orange-400 font-bold">Beds</p>
                          <p className="text-sm font-bold text-orange-900">{history.metadata_filters.bedrooms}</p>
                        </div>
                      </div>
                   )}
                </div>
